;; An enhaced on-chain counter that stores a count for each individual
;; with additional features for the Stacks Blockchain Bootcamp on risein.com
...
(define-read-only (get-total-operation) 
  (var-get total-ops)
)

;; Private function to update total aperations counter
(define-private (update-total-ops) 
  (var-set total-ops (+ (var-get total-ops) u1))
)

;; Functino to incremente the counter for the caller
...
;; Function to decrement the counter for the caller
(define-public (count-deon) 
  (let ((current-count (get-count tx-sender)))
